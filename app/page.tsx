...
const getDeployAwareMarkup = () => {
  ...
  const markup = injectPortfolioProjects(dreamWorldMarkup);
  if (!basePath) return markup;
  return markup.replace(/\s(src)=["']\/(?!\/)([^"']+)["']/g, ' $1="' + basePath + '/$2"');
};
...
const renderPortfolioProjects = () =>
  portfolioProjects
    .map((project) => {
      const revealClass = project.revealDelay ? " reveal-delay-" + project.revealDelay : "";
      const description = [
        escapeHtml(project.summary),
        "职责 · " + escapeHtml(project.responsibilities),
        "成果 · " + escapeHtml(project.results),
      ].join("<br><br>");
      const tags = project.tags.map((tag) => '          <span class="stack-tag">' + escapeHtml(tag) + "</span>").join("\n");
      const metrics = project.metrics.map((metric) => "<strong>" + escapeHtml(metric.value) + "</strong><span>" + escapeHtml(metric.label) + "</span>").join("");

      return `    <div class="project-ruin reveal${revealClass}" data-project="${escapeHtml(project.id)}">
      <div class="project-idx">${escapeHtml(project.index)}</div>
      <div>
        <div class="project-name">${escapeHtml(project.title)} · ${escapeHtml(project.period)}</div>
        <p class="project-desc">${description}</p>
        <div class="project-stack">
${tags}
        </div>
      </div>
      <div class="project-metrics" aria-hidden="true">${metrics}</div>
      <button class="project-cta">ENTER ↗</button>
    </div>`;
    })
    .join("\n\n");

const injectPortfolioProjects = (markup: string) => {
  ...
  return markup.slice(0, start) + startMarker + "\n" + renderPortfolioProjects() + markup.slice(end);
};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const musicIcon = musicToggle?.querySelector<HTMLElement>(".music-icon");
    ...
    bgm.loop = true;
    bgm.preload = "auto";
    bgm.volume = musicVolume ? Number(musicVolume.value) : 0.42;
    const mouse = { x: 0, y: 0 };

    const setMusicVisualState = (isPlaying: boolean) => {
      musicController?.classList.toggle("is-playing", isPlaying);
      if (musicToggle) {
        musicToggle.setAttribute("aria-pressed", String(isPlaying));
        musicToggle.setAttribute("aria-label", isPlaying ? "Pause background music" : "Play background music");
      }
      if (musicIcon) musicIcon.textContent = isPlaying ? "Ⅱ" : "▶";
      if (musicState) musicState.textContent = isPlaying ? "SIGNAL LIVE" : audioStarted ? "SIGNAL PAUSED" : "CLICK TO WAKE";
    };
    ...
    const updateMusicIntensity = () => {
      if (!audioAnalyser || !audioData || bgm.paused) {
        musicIntensity += (0 - musicIntensity) * 0.08;
        return;
      }
      ...
      for (let index = 0; index < audioData.length; index += 1) {
        const weight = index < 10 ? 1.35 : 0.72;
        sum += audioData[index] * weight;
      }
      const average = sum / audioData.length / 255;
      const target = Math.min(1, Math.pow(average, 0.78) * 1.45);
      musicIntensity += (target - musicIntensity) * 0.12;
    };
    ...
    class WaterParticle {
      ...
      update(t: number) {
        this.x += this.speed * (1 + musicIntensity * 2.4);
        if (this.x > width + 20) this.x = -20;
        this.y = waveYAt(this.x, this.waveIndex, t) + this.offsetY - musicIntensity * (3 + this.waveIndex * 0.45);
      }

      draw(t: number) {
        const flicker = 0.5 + (this.sparkle ? 0.56 : 0.4) * Math.sin(t * this.flickerSpeed + this.phase);
        const a = Math.max(0.04, Math.min(0.86, this.alpha * flicker * (1 + musicIntensity * 0.8)));
        const rhythmRadius = this.r * (1 + musicIntensity * (this.sparkle ? 1.7 : 1.05));
        const glow = context.createRadialGradient(this.x, this.y, 0, this.x, this.y, rhythmRadius * (this.sparkle ? 5.2 : 3.8));
        ...
        context.beginPath();
        context.arc(this.x, this.y, rhythmRadius, 0, Math.PI * 2);
    ...
    class FarStar {
      ...
      draw(t: number) {
        this.x += this.speed * (1 + musicIntensity * 1.6);
        if (this.x > width + this.r * 3) this.x = -this.r * 3;
        const starRadius = this.r * (1 + musicIntensity * 0.7);
        const a = Math.min(0.82, this.alpha * (0.5 + 0.5 * Math.sin(t * 0.015 + this.phase)) * (1 + musicIntensity * 0.9));
        const glow = context.createRadialGradient(this.x, this.y, 0, this.x, this.y, starRadius * 4.4);
        glow.addColorStop(0, "rgba(160,210,255," + a + ")");
        glow.addColorStop(0.18, "rgba(160,210,255," + (a * 0.28) + ")");
        glow.addColorStop(1, "rgba(160,210,255,0)");
        context.beginPath();
        context.arc(this.x, this.y, starRadius, 0, Math.PI * 2);
        context.fillStyle = glow;
        context.fill();
      }
    }
    ...
    const drawWaterWaves = (t: number) => {
      for (let index = 0; index < waveCount; index += 1) {
        ...
        gradient.addColorStop(0.48, "rgba(80,160,255," + (0.14 + index * 0.012 + musicIntensity * 0.08) + ")");
        ...
        context.lineWidth = 0.5 + (index % 3) * 0.18 + musicIntensity * 0.42;
        context.stroke();
      }
    };
    ...
    const sections = ["LOADER", "HERO", "ABOUT", "WORKS", "SKILL FOREST", "PROJECTS", "CONTACT"];
    const onScroll = () => {
    ...
    const handleDocumentClick = (event: MouseEvent) => {
      ...
      if (!audioStarted) void startMusic();
    ...
    musicVolume?.addEventListener("input", handleVolumeInput);
    ...
    return () => {
      ...
      musicVolume?.removeEventListener("input", handleVolumeInput);
